...
import { recursiveGenerateExportFile, generateExportFile } from '@autogen-export/core'
...
class AutogenExport extends Command {
  ...
  static flags = {
    cfg: flags.boolean({
      char: 'c',
      description: 'specify configuration file. If specifiied, all agurments except directory will be ignored ',
    }),
  ...
  static args = [
    ...
    },
    {
      ...
      description: 'Path which matched regex will be ignored (use in recursive mode)',
      default: undefined,
    },
    {
      name: 'generatedFileExt',
      description: 'Extension of generated file. Generated file name will be index',
      default: 'ts',
    },
    {
  ...
  async run() {
    ...
    if (!cfg && !rootDirectory) {
      throw new Error('If not use config file then root directoy must be specify')
    }

    let transformedIgnoreDestinationRegexs = transformRegexesString(ignoreDestinationRegexs)

    let transformedIgnoreMatchFileRegexes = transformRegexesString(ignoreMatchFileRegexes)
